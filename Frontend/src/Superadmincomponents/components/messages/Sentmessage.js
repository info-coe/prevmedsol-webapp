...
const SuperadminSentmessage = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        res.data.map((item) => {
          if (superadmin.email === item.fromemail) {
                ...
                if((index) === (clickedindex)){
                  return (
                    <div key={index}>
                      ...
                      <p><b>To : </b><Link to="/superadminmessage" state={{email : item.toemail}}>{item.toemail}</Link></p>
...
export default SuperadminSentmessage;